...
-- [[ START FIX ]] --
-- Helper: Proper Snake Case handling acronyms (e.g., "UserID" -> "user_id", "ISBN" -> "isbn")
local function to_snake_case(str)
	-- 1. Handle transitions from Acronym to Word (e.g., "HTTPServer" -> "HTTP_Server")
	local result = str:gsub("(%u+)(%u%l)", "%1_%2")
	-- 2. Handle transitions from Lower to Upper (e.g., "ServerID" -> "Server_ID")
	result = result:gsub("(%l)(%u)", "%1_%2")
	return result:lower()
end

-- Helper: Proper Camel Case (e.g., "ID" -> "id", "ServerID" -> "serverID")
local function to_camel_case(str)
	-- If the word is entirely uppercase (like "ID", "ISBN", "URL"), return it lowercased (common Go JSON convention)
	if str:match("^%u+$") then
		return str:lower()
	end
	-- Otherwise, lowercase just the first character
		...
		if not field.embedded and #field.names > 0 then
			...
			if line_idx >= 1 and line_idx <= #lines then
				...
				for _, tag_type in ipairs(tag_types) do
					...
					-- [[ START FIX ]] --
					local tag_value = field_name

					-- Determine transform strategy based on tag type or config
					-- JSON usually prefers camelCase, DB/YAML usually prefer snake_case
					local transform_type = cfg.transform
					if not transform_type then
						if tag_type == "db" or tag_type == "yaml" then
							transform_type = "snakecase"
						else
							transform_type = "camelcase"
						end
					end

					if transform_type == "snakecase" then
						tag_value = to_snake_case(field_name)
					elseif transform_type == "camelcase" then
						tag_value = to_camel_case(field_name)
					else
						-- Fallback
						tag_value = util.transform_name(field_name, cfg.transform)
					end
					-- [[ END FIX ]] --
