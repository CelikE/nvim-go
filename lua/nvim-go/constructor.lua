...
function M.build_constructor(struct, opts)
  ...
  if add_comment then
    ...
    table.insert(
      lines,
      string.format("// %s creates a new %s instance.", func_name, struct_name)
    )
  else
    table.insert(lines, "")
  end
...
function M.build_functional_options_constructor(struct)
  ...
  table.insert(
    lines,
    string.format("// %s is a functional option for %s.", opt_type, struct_name)
  )
  ...
  for _, field in ipairs(struct.fields) do
    if not field.embedded then
      for _, name in ipairs(field.names) do
        ...
        table.insert(
          lines,
          string.format("// %s sets the %s field.", func_name, name)
        )
        table.insert(
          lines,
        ...
        table.insert(
          lines,
          string.format("\treturn func(%s *%s) {", receiver, struct_name)
        )
  ...
  table.insert(
    lines,
    string.format("// New%s creates a new %s with the given options.", struct_name, struct_name)
  )
  table.insert(
    lines,
    string.format("func New%s(opts ...%s) *%s {", struct_name, opt_type, struct_name)
  )
